...
library(xfun)
pkg_attach2("pdftools", "tidyverse", "rvest", "httr", "roperators", "quanteda",
            "xml2")
...
# parse PDF in R [REWRITE!!!!]

Use the [tabulizer](https://github.com/ropensci/tabulizer) package to parse PDFs in R.

⚠️ This package requires [rJava](https://cran.r-project.org/web/packages/rJava/index.html). This can be a real pain to install. See [tabulizer README](https://github.com/ropensci/tabulizer/README.md). However, tabulizer is more powerful for extracting tables from PDF docs.

If you can't get tabularizer to install, you can also use [pdftools](https://docs.ropensci.org/pdftools/) package to parse PDFs in R. This does not require rJava, but is less powerful for extracting tables from PDFs.

---

# Download PDF and extract table [REWRITE!!!!]

```{r download-pdf, cache=TRUE, warning=FALSE, message=FALSE, eval=FALSE}
# library(tabulizer)

# url <- "https://www.visionofhumanity.org/wp-content/uploads/2020/10/GPI_2020_web.pdf"
...
# Note: you can also extract text data from PDFs [REWRITE!!!!]
