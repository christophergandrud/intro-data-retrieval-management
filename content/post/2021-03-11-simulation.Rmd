...
Imagine that we want to run an A/B test--randomised control trial--a new treatment to the status quo control treatment. We are interested in the effect of the treatment on whether or not individuals in the study do some action--e.g. vote in an election.[^1]
...
It's unlikely that we will run an A/B test with only one participant in the control group, let's go up to 20,000[^2] and plot the results.

[^2]: We can express this in R using scientific notation, e.g. `2e4`.

```{r}
control <- rbinom(n = 2e4, size = 1, prob = 0.1)
hist(control)
```
...
control_tbl <- as_tibble(control)

# Plot and change the x-axis label
ggplot(control_tbl, aes(value)) + # as_tibble names the column "value" by default
  geom_histogram() + 
...
Ok, we've simulated one run of the control group. Now let's think about the treatment group. Imagine we anticipate that a realistic effect of the treatment--again based on prior knowledge--is a relative improvement of 1%. Let's

We can begin filling in our function to run the simulation one time:

```{r}
one_sim = function(n = 1000, prob = 0.2) {
