...
Debugging is the process of finding and resolving discrepancies between a program's specification and its implementation.[^debug] For example, I want a ggplot2 to plot multiple time series on the same plot as separate lines, but it plots them all as one--very jagged and confusing line. I could use a debugging process to find the cause of and correct this discrepancy.

[^debug]: See the [Wiki page on debugging](https://en.wikipedia.org/wiki/Debugging) for a visual example of an early computer bug, an actual bug that got stuck in a computer. 
...
- create examples and automated tests to ensure that the functions run 
correctly using the `testthat` package. Hint: install the *usethis* package and, within your package's R project enter the function `use_testthat()`.  
